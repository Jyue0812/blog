...
```python
	...
	#4.使用 vgextend 命令把/dev/vdc1加入到centos磁盘组
		vgextend vg_centos68 /dev/sdb1 
		#使用 vgdisplay 查看卷组信息，下图显示卷组名为centos，空闲大小为0：
		lvdisplay
		
	#5.使用 lvcreate 命令从卷组里划分一个新的逻辑卷，这里创建了名称为hal，大小1000MB的逻辑卷分区；
		lvcreate -L 1000M -n sdb1 vg_centos68
		#使用lvdisplay 查看逻辑卷信息：
		lvdisplay 

	#####如果VG有未知错误
	#vgreduce --removemissing  磁盘组名
	vgreduce --removemissing vg_centos68
		
	#6.格式化分区
		#将分区格式为你所需要的格式   ext4
        #mkfs.xfs /dev/组名/磁盘的别名
		mkfs.xfs /dev/vg_centos68/sdb1
		
		#挂载分区
		mount  /dev/vg_centos68/sdb1 /mnt/dev1
	...
	7.设置开机挂载
		vim /etc/fstab
		添加一行
		/dev/vg_centos68/sdb1     /boot     xfs        defaults        0 0
		设备名         挂载点        系统格式     默认           默认
